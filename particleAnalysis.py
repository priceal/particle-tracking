...
##############################################################################
##############################################################################
def finderCNN( image, cnn, imgDF = None, border=False, bx=3, by=3):
    """
    find peaks in a provided image. image should be a PIL image. 
    finds peaks by locating maximum intensity, then setting surrounding
    box to zero and repeating.
    ...
    scaledImage = image/image.max()
    yFrame, xFrame = scaledImage.shape
    
# Applying CNN: converting image to tensor, then detaching output, converting back
# to array and padding to have same dimensions as input image
    x = torch.FloatTensor( scaledImage[np.newaxis,np.newaxis,:,:] )
    mapout = cnn( x ).detach().numpy()
    squeezedMap = np.squeeze( mapout )
   
    yMap, xMap = squeezedMap.shape
    bx = int((xFrame-xMap)/2)
    by = int((yFrame-yMap)/2)
    mapFinal = cv.copyMakeBorder( \
          squeezedMap , by, by, bx, bx, cv.BORDER_CONSTANT, 0)
